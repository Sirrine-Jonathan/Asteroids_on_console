...
{
   int topMargin = 20;
   int numbersY = topLeft.getY() - topMargin;
   int labelsY = topLeft.getY() - 28;

   //display shield time
   int centerOffset = 90;
   int totalWidth = 150;
   int valueWidth = 50;   
   Point p1(bottomRight.getX() - totalWidth, labelsY);
   drawText(p1, "Shield Time: ");	
   Point p2(bottomRight.getX() - valueWidth, numbersY);
   drawNumber(p2, getPowers());

   //display score
   Point p3(-35, labelsY);
   drawText(p3, "Score: ");
   Point p4(25, numbersY);
   drawNumber(p4, getScore());

   //display time
   Point p5(topLeft.getX() + 40, labelsY);
   drawText(p5, "Game Time: ");
   Point p6(topLeft.getX() + 40 + 85, numbersY);
   drawNumber(p6, getTime());
}
